...
parser.add_argument(
    "--var_len",
    action="store_true",
    help="Use variable length input",
)
parser.add_argument(
    "--deterministic",
    action="store_true",
...
parser.add_argument(
    ...
    type=int,
    default=None,
    help="sliding_window_length",
)

# nees to be consistent with benchmark_attn.py
...
if args.sdpa_backend == "cudnn_fe":
    ...
    if args.var_len:
        drop_num = torch.tensor(var_seq_drop_token_num[:batch_size], device=device, dtype=torch.int32)
        seq_len_q_gpu = (torch.ones(batch_size, device=device, dtype=torch.int32) * q_seqlen - drop_num).reshape(batch_size, 1, 1, 1)
        seq_len_kv_gpu = (torch.ones(batch_size, device=device, dtype=torch.int32) * kv_seqlen - drop_num).reshape(batch_size, 1, 1, 1)
    ...
    if args.data_type == "fp8":
        ...
        if args.var_len:
            seq_len_q_fwd = graph_fwd.tensor_like(seq_len_q_gpu)
            seq_len_kv_fwd = graph_fwd.tensor_like(seq_len_kv_gpu)
        else:
            seq_len_q_fwd = None
            seq_len_kv_fwd = None
        o_fwd, stats_fwd = graph_fwd.sdpa(
            ...
            k=k_fwd,
            v=v_fwd,
            use_padding_mask=args.var_len,
            seq_len_q=seq_len_q_fwd,
            seq_len_kv=seq_len_kv_fwd,
            # generate_stats=not is_infer,
            is_inference=is_infer,
            attn_scale=attn_scale,
            diagonal_alignment=(
            ...
            dropout=dropout_tuple if is_dropout else None,
            diagonal_band_left_bound=args.window_size,
        )
    ...
    if args.fwd_bwd:
        ...
        if args.data_type == "fp8":
            ...
            if args.var_len:
                seq_len_q_bwd = graph_bwd.tensor_like(seq_len_q_gpu)
                seq_len_kv_bwd = graph_bwd.tensor_like(seq_len_kv_gpu)
            else:
                seq_len_q_bwd = None
                seq_len_kv_bwd = None
            ...
            dQ_bwd, dK_bwd, dV_bwd = graph_bwd.sdpa_backward(
                ...
                diagonal_band_right_bound=None if args.attn_mask == "no_mask" else 0,
                dropout=dropout_tuple if is_dropout else None,
                use_deterministic_algorithm=args.deterministic,
                diagonal_band_left_bound=args.window_size,
            )
        ...
        if args.data_type == "fp8":
            ...
            if args.var_len:
                variant_pack_fwd[seq_len_q_fwd] = seq_len_q_gpu
                variant_pack_fwd[seq_len_kv_fwd] = seq_len_kv_gpu
                variant_pack_bwd[seq_len_q_bwd] = seq_len_q_gpu
                variant_pack_bwd[seq_len_kv_bwd] = seq_len_kv_gpu
            workspace = torch.empty(
                max(graph_fwd.get_workspace_size(), graph_bwd.get_workspace_size()),
                device="cuda",
                dtype=torch.uint8,
...
if args.fwd_bwd:
    ...
    if args.format_output:
        print(
            f"{args.case_tag},{args.sdpa_backend},{args.batch_size},{args.q_seqlen},{args.kv_seqlen},{args.num_q_heads},{args.num_kv_heads},{head_dim_qk},{fwd_median_time:.3f},{bwd_median_time:.3f},{fwd_tflops:.0f},{bwd_tflops:.0f},{forward_diffs[0]:.6f},{num_iters}"
        )
    else:
        print(
            f"{args.sdpa_backend}:: Median (fwd, bwd) Execution Times: {fwd_median_time:.3f} ms ({fwd_tflops:.0f} TFLOPS), {bwd_median_time:.3f} ms ({bwd_tflops:.0f} TFLOPS) (max difference vs. pyt_reference: {forward_diffs[0]:.6f} from {num_iters} iterations)"
        )
else:
    if args.format_output:
        print(
            f"{args.case_tag},{args.sdpa_backend},{args.batch_size},{args.q_seqlen},{args.kv_seqlen},{args.num_q_heads},{args.num_kv_heads},{head_dim_qk},{fwd_median_time:.3f},0,{fwd_tflops:.0f},0,{forward_diffs[0]:.6f},{num_iters}"
        )
    else:
        print(
            f"{args.sdpa_backend}:: Median (fwd) Execution Times: {fwd_median_time:.3f} ms ({fwd_tflops:.0f} TFLOPS) (max difference vs. pyt_reference: {forward_diffs[0]:.6f} from {num_iters} iterations)"
        )